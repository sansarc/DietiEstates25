...
public class DetailsForm extends Form {
    ...
    protected void createComponents() {
        ...
        var parkingInfo = new InfoPopover(garageSpots, "In this category are NOT included parking spots in the road or lots in stores or parks nearby, ONLY parking spots within the property. You may add this in the description field.");
        var disabledAmenities = new Checkbox("Disabled amenities");
// Create a custom Select with colored items
        Select<String> energyClassSelect = new Select<>();
//        energyClassSelect.setRenderer(new ComponentRenderer<>(item -> {
//            Span span = new Span(item);
//            span.getStyle().set("color", getEnergyClassColor(item));
//            return span;
//        }));
